  ...
  return (
    <main className="product-page">
      <div className="container">
        <Link href="/collection" className="back"><ArrowLeft size={16} /> Back to collection</Link>

        <div className="product-detail">
          <div className="product-detail-gallery">
            {product.images.map((image) => (
              <figure key={image.id} className="product-detail-image">
          ...
          <aside className="product-detail-info">
            ...
            {product.price !== undefined && (
              <p className="product-price">{product.currency ?? "NPR"} {product.price.toLocaleString()}</p>
            )}

            {product.sizes?.length ? (
              <div className="detail-group">
                <span className="detail-label">Sizes</span>
                <div className="detail-values">{product.sizes.map((size) => <span key={size}>{size}</span>)}</div>
              </div>
            ) : null}
            ...
            {product.colors?.length ? (
              <div className="detail-group">
                ...
                <div className="detail-values">{product.colors.map((color) => <span key={color}>{color}</span>)}</div>
              </div>
            ) : null}
