...
export default function HomePage() {
  ...
  return <main>
    ...
    <section id="collection" className="section collection"><div className="container"><div className="section-head"><div><p className="eyebrow">Curated for discovery</p><h2 className="section-title">Explore the Collection</h2></div><a className="btn btn-light" href="/collection">View Lookbook <ArrowUpRight size={16}/></a></div><div className="category-grid">{categories.map((category,i)=><a href="/collection" className={`category category-${i+1}`} key={category.id} onClick={()=>track("collection_click", { category: category.id })}><div className="category-photo" style={{backgroundImage:`url(${category.image?.src ?? products[i%Math.max(products.length,1)]?.images[0]?.src ?? fallbackHero})`}}/><div className="category-copy"><span>{category.name}</span><ArrowUpRight size={18}/></div></a>)}</div></div></section>

    <section id="lookbook" className="section lookbook"><div className="container"><div className="section-head"><div><p className="eyebrow">Featured lookbook</p><h2 className="section-title">Pieces worth a closer look.</h2></div><p className="muted lookbook-note">Inventory is not connected yet. Ask the store about availability.</p></div><div className="product-masonry">{products.map((product,i)=><article className={`product product-${i+1}`} key={product.id}><div className="product-image"><img src={product.images[0]?.src} alt={product.images[0]?.alt ?? product.name} loading={i < 2 ? "eager" : "lazy"}/><a href={whatsapp(store?.whatsappNumber ?? "9779864831830", `Hi Fashion Factory, I would like to ask about ${product.name}. Is it currently available?`)} target="_blank" rel="noreferrer" className="product-action" onClick={()=>track("product_click", { product: product.id })}>Ask for Availability <ArrowUpRight size={15}/></a></div><div className="product-meta"><div><p>{product.name}</p><small>{categories.find(c=>c.id===product.categoryId)?.name}</small></div>{product.price !== undefined ? <span>{product.currency ?? "NPR"} {product.price.toLocaleString()}</span> : null}</div></article>)}</div></div></section>
    ...
    <section id="contact" className="section contact"><div className="container contact-grid"><div><p className="eyebrow">Talk to the store</p><h2 className="section-title">Have a question about a look?</h2><p className="muted">Send an inquiry and the store can follow up using the contact information you provide.</p></div><ContactForm/></div></section>
